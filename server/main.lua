...
ESX.RegisterServerCallback('esx_society:setJob', function(source, cb, identifier, job, grade, type)
	...
	if xPlayer ~= nil then
		...
		if type == 'hire' then
          ...
          MySQL.Async.execute(
            'INSERT INTO whitelist_jobs SET job = @job, grade = @grade, identifier = @identifier',
            {
              ['@job']    = job,
              ['@grade']  = grade,
		...
		elseif type == 'promote' then
          ...
          MySQL.Async.execute(
            'UPDATE whitelist_jobs SET job = @job, grade = @grade WHERE identifier = @identifier',
            {
              ['@job']    = job,
              ['@grade']  = grade,
              ['@identifier'] = identifier
		...
		elseif type == 'fire' then
          ...
          MySQL.Async.execute(
            'DELETE FROM whitelist_jobs WHERE job = @job AND identifier = @identifier',
            {
              ['@job']        = job,
              ['@identifier'] = identifier
            },
